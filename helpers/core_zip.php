...
class Core_Zip
{
	...
	public static function init_zip()
	{
		...
		if (!defined('PATH_INSTALL'))
			require_once(PATH_APP."/modules/core/vendor/pclzip/pclzip.lib.php");
...
function zip_helper_post_extract($p_event, &$p_header)
{
	if (file_exists($p_header['filename']))
	{
		...
		if (!Core_Zip::$chmod_error)
		{
			$mode = $is_folder ? Phpr_Files::get_folder_permissions() : Phpr_Files::get_file_permissions();
			try
			{
